...
@router.post("/generate-embeddings-youtube/")
async def generate_embeddings(request: EmbeddingRequest):
    try:
        ...
        video_id = extract_video_id(request.content)
        print(f"ID do vídeo extraído: {video_id}")

        # Buscar a transcrição em português (ou idiomas alternativos)
        try:
            transcription = YouTubeTranscriptApi.get_transcript(video_id, languages=['pt'])
        except NoTranscriptFound:
            raise HTTPException(
                status_code=404,
                detail="Nenhuma transcrição disponível em português para este vídeo."
            )
        except TranscriptsDisabled:
            raise HTTPException(
                status_code=403,
                detail="As transcrições estão desabilitadas para este vídeo."
            )

        # Unir os textos das transcrições
        ...
        print(f"Texto da transcrição (primeiros 200 caracteres): {full_text[:200]}...")
