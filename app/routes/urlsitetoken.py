...
@router.post("/generate-embeddings-url/")
async def generate_embeddings(request: EmbeddingRequest):
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(
                status_code=400,
                detail="Erro ao acessar o site. Verifique o endereço fornecido."
            )

        # Extrair texto do HTML usando BeautifulSoup
        soup = BeautifulSoup(response.text, "html.parser")
        site_text = soup.get_text(separator="\n")  # Extrair texto visível no site

        # Normalizar o texto para garantir consistência
        site_text = unicodedata.normalize("NFKC", site_text)
        print(f"Texto inicial do site: {site_text[:200]}...")  # Log para debug

        # Dividir o texto em pedaços
        chunks = text_splitter.split_text(site_text)
